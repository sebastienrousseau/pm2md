...
describe("Test 'readMethods'", () => {
  test("Check 'readMethods' function", () => {
    ...
    readMethods(data);
    expect(data.name || data.response || data.request.description || data.request.method || data.request.url || data.request.body || data.request.auth || data.request.header || data.request.key || data.request.value).toEqual("string");
  });
});
